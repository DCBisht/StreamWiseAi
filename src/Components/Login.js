  ...
  return (
    <div>
      ...
      className=' bg-opacity-80 absolute rounded-lg p-14 bg-black w-4/12 my-36 mx-auto right-0 left-0 text-white'>
        ...
        {!isSignInForm && <input type='text' placeholder='Full Name' className='rounded-lg p-2 m-2 w-full bg-gray-800' ref={name}/>}
        <input type='text' placeholder='Email address' className='rounded-lg p-2 m-2 w-full bg-gray-800' ref={email}/>
       
        <input type='password' placeholder='Password' className='rounded-lg p-2 m-2 w-full bg-gray-800' ref={password}/>
        <p className='p-1 my-1 mx-2 text-red-500 font-bold'>{errorMessage}</p>
        <button className='rounded-lg p-2 m-2 mx-3 bg-red-700 w-full' 
        onClick={handleButtonClick}>{isSignInForm?"Sign In":"Sign Up"}</button>
        
        <p className='p-1 cursor-pointer' onClick={toggleSignUP}>{isSignInForm?"New to platform? Sign Up Now":"Already a user? Sign In"}</p>
      </form>
    </div>
  )
}
